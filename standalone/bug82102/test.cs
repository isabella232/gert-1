...
class TinyHost : MarshalByRefObject
{
	...
	static int Main ()
	{
		...
		if (result.IndexOf ("Mono ASP.NET Rulez!") == -1) {
			Console.WriteLine (result);
			return 2;
		}
#if NET_2_0
		if (result.IndexOf ("<link rel=\"alternate\" type=\"text/xml\" href=\"&lt;%=Request.FilePath%>?disco\" />") == -1) {
#else
		if (result.IndexOf ("<link rel=\"alternate\" type=\"text/xml\" href=\"/Default.aspx?disco\"/>") == -1) {
#endif
			Console.WriteLine (result);
			return 2;
		}

...
class TinyHost : MarshalByRefObject
{
	...
	static int Main ()
	{
		...
		string hostDir = Path.Combine (AppDomain.CurrentDomain.BaseDirectory, "web");
		StreamWriter w = new StreamWriter (Path.Combine (hostDir, "page.aspx"));
		w.WriteLine (@"<%@ Page Language=""C#"" %>");
		w.WriteLine (@"Hello");
		w.Close ();
		StringWriter sw = new StringWriter ();
		h.Execute ("page.aspx", sw);

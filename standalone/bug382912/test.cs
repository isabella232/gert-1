...
class Program
{
	...
	static int RunTest (SqlConnection conn)
	{
		...
		cmd = new SqlCommand ("INSERT INTO bug382912 VALUES ('Mono')", conn);
		cmd.ExecuteNonQuery ();
		cmd.Dispose ();
		...
		cmd = new SqlCommand ("UPDATE bug382912 SET Name = 'Rocks'", conn);
		cmd.ExecuteNonQuery ();

		cmd = new SqlCommand ("SELECT * FROM bug382912", conn);
		using (SqlDataReader dr = cmd.ExecuteReader ()) {
			Assert.IsTrue (dr.Read (), "#C1");
			Assert.AreEqual ("Rocks", dr.GetString (0), "#C2");
